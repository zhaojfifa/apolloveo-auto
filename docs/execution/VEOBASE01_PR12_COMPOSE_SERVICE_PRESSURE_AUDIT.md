## PR-12: compose_service.py pressure audit and safe boundary extraction

Branch name:

- `VeoBase01-pr12-compose-service-pressure-audit`

Base SHA:

- `796472577fa639d9c95ecffe410ed950b5051a8f`

Purpose:

- reduce pure-helper concentration in `gateway/app/services/compose_service.py`
- prevent the compose service from continuing to absorb subtitle-render policy helpers
- keep orchestration, workspace prep, worker execution, and FFmpeg command behavior unchanged

Scope:

- extract pure subtitle-render/layout/filter helpers into `gateway/app/services/compose_subtitle_rendering.py`
- keep `compose_service.py` as the orchestration owner and re-export consumer of those helpers
- add direct tests for the extracted helper module using the repo’s current behavior

Non-goals:

- no FFmpeg behavior change
- no compose policy change
- no worker/runtime policy change
- no retune of subtitle default sizes or signatures
...
- `git diff --check`
- `PYTHONPYCACHEPREFIX=/tmp/apolloveo_pycache python3.11 -m py_compile gateway/app/services/compose_service.py gateway/app/services/compose_subtitle_rendering.py gateway/app/services/tests/test_compose_subtitle_rendering.py`
...
- Exact live task data for `9c755859d049` and `9280fcb9f0b1` is not available in this workspace, so PR-12 reran the same closest-equivalent read-only synthetic in-memory probe used by the earlier VeoBase01 refactor PRs.
...
- `compose_service.py` still contains the command-branch implementations themselves; this PR only removes one pure helper cluster
- the known subtitle-render default tests remain pre-existing and intentionally untouched

Rollback path:

- `git checkout VeoBase01`
- `git merge --ff-only 796472577fa639d9c95ecffe410ed950b5051a8f`
- or revert the PR-12 commit if only the helper extraction must be undone

Acceptance judgment:

- accepted for PR-12 scope
- `compose_service.py` reduced from `2348` to `2111` lines
- pure subtitle-render helper pressure moved out without changing compose orchestration ownership
- no FFmpeg behavior, worker behavior, or runtime policy was intentionally changed
