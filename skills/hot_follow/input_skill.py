...
def run(
    ...
    *,
    ...
    subtitle_ready = bool(current_attempt.get("subtitle_ready") or ready_gate.get("subtitle_ready"))
    ...
    return {
        ...
        "subtitle_ready_reason": str(
            current_attempt.get("subtitle_ready_reason") or ready_gate.get("subtitle_ready_reason") or ""
        ).strip()
        or None,
        ...
        "compose_allowed": bool(current_attempt.get("compose_allowed") or ready_gate.get("compose_allowed")),
        "compose_route_allowed": bool(
            current_attempt.get("compose_route_allowed") or ready_gate.get("compose_route_allowed")
        ),
        "compose_input_ready": bool(
            current_attempt.get("compose_input_ready") or ready_gate.get("compose_input_ready")
        ),
        "compose_execute_allowed": bool(
            current_attempt.get("compose_execute_allowed") or ready_gate.get("compose_execute_allowed")
        ),
        "compose_input_mode": str(
            current_attempt.get("compose_input_mode") or ready_gate.get("compose_input_mode") or ""
        ).strip()
        or None,
        "compose_input_reason": str(
            current_attempt.get("compose_input_reason") or ready_gate.get("compose_input_reason") or ""
        ).strip()
        or None,
        ...
        "compose_blocked_reason": str(
            current_attempt.get("compose_reason") or ready_gate.get("compose_blocked_reason") or ""
        ).strip()
        or None,
        "compose_input_derive_failed_terminal": bool(
            current_attempt.get("compose_input_derive_failed_terminal")
            or ready_gate.get("compose_input_derive_failed_terminal")
        ),
        "compose_input_blocked_terminal": bool(
            current_attempt.get("compose_input_blocked_terminal")
            or ready_gate.get("compose_input_blocked_terminal")
        ),
        "compose_exec_failed_terminal": bool(
            current_attempt.get("compose_exec_failed_terminal")
            or ready_gate.get("compose_exec_failed_terminal")
        ),
        ...
        "no_dub_reason": str(current_attempt.get("no_dub_reason") or ready_gate.get("no_dub_reason") or "").strip() or None,
