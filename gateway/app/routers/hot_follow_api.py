...
from gateway.app.services.task_view import (
    ...
    collect_hot_follow_workbench_ui as _service_collect_hot_follow_workbench_ui,
    hf_deliverables as _service_hf_deliverables,
    hf_pipeline_state as _service_hf_pipeline_state,
    hf_task_status_shape as _service_hf_task_status_shape,
    safe_collect_hot_follow_workbench_ui as _service_safe_collect_hot_follow_workbench_ui,
)
...
def _hf_pipeline_state(task: dict, step: str, *, composed: dict[str, Any] | None = None) -> tuple[str, str]:
    return _service_hf_pipeline_state(
        task,
        step,
        composed=composed,
        state_from_status=_hf_state_from_status,
        parse_artifact_ready_loader=_hf_parse_artifact_ready,
        voice_state_loader=_collect_voice_execution_state,
...
def _hf_deliverables(task_id: str, task: dict) -> list[dict[str, Any]]:
    return _service_hf_deliverables(
        task_id,
        task,
        task_key_loader=_task_key,
        subtitle_lane_loader=_hf_subtitle_lane_state,
        voiceover_asset_loader=_hf_current_voiceover_asset,
        settings_loader=get_settings,
...
def _hf_task_status_shape(task: dict) -> dict[str, str]:
    return _service_hf_task_status_shape(
        task,
...
def _collect_hot_follow_workbench_ui(task: dict, settings) -> dict[str, Any]:
    return _service_collect_hot_follow_workbench_ui(
        task,
        settings,
        subtitle_lane_loader=_hf_subtitle_lane_state,
        pipeline_state_loader=_hf_pipeline_state,
        dual_channel_loader=_hf_dual_channel_state,
...
def _safe_collect_hot_follow_workbench_ui(task: dict, settings) -> dict[str, Any]:
    return _service_safe_collect_hot_follow_workbench_ui(
        task,
        settings,
        collector=_collect_hot_follow_workbench_ui,
        ...
        voice_options_loader=_build_hot_follow_voice_options,
    )
